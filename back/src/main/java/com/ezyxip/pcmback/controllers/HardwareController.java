...
public class HardwareController {
    ...
    public ResponseEntity<GPUEntity> createGPU(@RequestBody GPUEntity gpu) {
        try {
            GPUEntity _gpu = gpuRepository
                    .save(new GPUEntity(gpu.getTitle()));
    ...
    public ResponseEntity<RAMEntity> createRAM(@RequestBody RAMEntity gpu) {
        try {
            RAMEntity _gpu = ramRepository
                    .save(new RAMEntity(gpu.getTitle()));
            return new ResponseEntity<>(_gpu, HttpStatus.CREATED);
        } catch (Exception e) {
            return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<HDDEntity> createGPU(@RequestBody HDDEntity gpu) {
        try {
            HDDEntity _gpu = hddRepository
                    .save(new HDDEntity(gpu.getTitle()));
            return new ResponseEntity<>(_gpu, HttpStatus.CREATED);
        } catch (Exception e) {
            return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<MotherboardEntity> createMotherboard(@RequestBody MotherboardEntity gpu) {
        try {
            MotherboardEntity _gpu = motherboardRepository
                    .save(new MotherboardEntity(gpu.getTitle()));
            return new ResponseEntity<>(_gpu, HttpStatus.CREATED);
        } catch (Exception e) {
            return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
        }
